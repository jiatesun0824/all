...
- (WXMediaMessage *)buildSharingMessage:(NSDictionary *)message
{
    ...
    switch (type)
    {
        ...
        case CDVWXSharingTypeMiniProgram:
            NSLog(@"分享类型type===> %d", type);
            mediaObject = [WXMiniProgramObject object];
            ((WXMiniProgramObject*)mediaObject).webpageUrl = [media objectForKey:@"webpageUrl"];
            ((WXMiniProgramObject*)mediaObject).userName = @"gh_5815820699d4";
            // ((WXMiniProgramObject*)mediaObject).miniProgramType = 0;// WXMiniProgramTypeRelease;
            ((WXMiniProgramObject*)mediaObject).path = [media objectForKey:@"path"];
//            ((WXMiniProgramObject*)mediaObject).hdImageData = [self getUIImageFromURL:[message objectForKey:@"thumb"]];
            NSLog(@"分享内容====> %@", mediaObject);
            break;
