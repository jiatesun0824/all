...
const dom = {
  ...
  replaceTag($el){
    let parent=$el.parentNode,$h;
    //console.log($el.getAttribute('style'))
    try {
      if($el.getAttribute('style')){

        if($el.getAttribute('style')=='font-weight: 800'){
          $el.setAttribute('style','font-weight: normal')
        }else {
          $h=document.createElement('p');
          $h.setAttribute('style','font-weight: 800')
          if($h.getAttribute('style')=='font-weight: 800'){
            $h.getAttribute('style')=='font-weight: normal'
          }else {
            $h.setAttribute('style','font-weight: 800')
          }
  ...
  addTitle($el,calkback){
    let div= document.getElementsByClassName('title-button');
    let div1= document.getElementsByClassName('demo-bold-button');
    div[div.length-1].style.opacity==0.3 ? div[div.length-1].style.opacity=1 : div[div.length-1].style.opacity=0.3;
    div1[div1.length-1].style.opacity=1;
    if($el){
      let $sty=$el.getAttribute('style');
      if($sty && $sty=='font-weight: 800;font-size :15px'){
        $el.setAttribute('style','font-weight: normal;');
      }else {
        $el.setAttribute('style','font-weight: 800;font-size :15px');
      }
    }
    calkback()
  },
  addBold($el,calkback){
      let div= document.getElementsByClassName('demo-bold-button');
      let div1= document.getElementsByClassName('title-button');
      div[div.length-1].style.opacity==0.3 ? div[div.length-1].style.opacity=1 : div[div.length-1].style.opacity=0.3;
      div1[div1.length-1].style.opacity=1;
      if($el){
        let $sty=$el.getAttribute('style');
        if($sty && $sty=='font-weight: 800'){
          $el.setAttribute('style','font-weight: normal');
        }else {
          $el.setAttribute('style','font-weight: 800');
        }
      }
      calkback();
