...
import DomRange from "./range";

// 编辑器默认图标
const TOOL_BAR_ICONS = ['pic', 'emoji', 'text', 'link','bold','title']
// toolbar配置
...
TOOL_BAR_OPTIONS.bold={
  name: 'bold',
  class: 'demo-bold-button',
  icon: '',
  on: 'blod-button'
}
TOOL_BAR_OPTIONS.title={
  name: 'title',
  class: 'title-button',
  icon: '',
  on: 'title-button'
  ...
  function toolbarChildClickHandler (e) {
    ...
    switch (name) {
      ...
      case 'bold':   //加粗
        ...
        dom.addBold(_this.$cursorElm,function () {
          try {
            _this.cursor.setRange(_this.$cursorElm,_this.cursor.range.startOffset);//设置光标
          }catch (e) {
            _this.cursor.setRange(_this.$cursorElm,0);//设置光标
          }
        });
         break
      case 'title': //标题

        dom.addTitle(_this.$cursorElm,function () {
          try {
            _this.cursor.setRange(_this.$cursorElm,_this.cursor.range.startOffset);//设置光标
          }catch (e) {
            _this.cursor.setRange(_this.$cursorElm,0);//设置光标
