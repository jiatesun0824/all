// pages/publishArticles/publishArticles.js
...
Page({
  ...
  publish(){
    ...
    let picId = this.data.addImgId.join(',')
    let params
    if(this.data.planFlag){
      params = {
        ...
        coverPicId:this.data.addImgId[0],
        bedroomNum: this.data.spaceIndex[0]+1,
        livingRoomNum: this.data.spaceIndex[1] + 1,
        houseArea:this.data.area,
        houseCost:this.data.price,
        houseStytle: this.data.styleList[this.data.styleIndex].styleCode,
        planCoverId: this.data.houseTypeImgId
      }
    }else{
      params={
        title:this.data.title,
